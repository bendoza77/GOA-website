...
export class JourneyEngine {
  constructor(canvas, { isDark = true, coarse = false, storyVh = null, sleepVh = null } = {}) {
    this.canvas = canvas;
    this.disposed = false;
    /* Beyond this many viewport-heights of scroll the ride is far behind the
       viewer (the cube act and then the world own the frame): skip all work
       until they scroll back. */
    this._sleepVh = sleepVh;
    this._asleep = false;
  ...
  _tick(now) {
    ...
    if (this._sleepVh && window.scrollY > this._sleepVh * window.innerHeight) {
      if (!this._asleep) {
        this.renderer.clear();
        this._asleep = true;
      }
